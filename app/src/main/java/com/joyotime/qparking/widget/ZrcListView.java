...
public class ZrcListView extends ZrcAbsListView {
	...
	void fillGap(boolean down) {
		...
		if (down) {
			...
			final int startOffset = count > 0 ? getChildAt(count - 1).getBottom() + mDividerHeight
					: mFirstTop + mListPadding.top + mFirstTopOffset;
			...
			for (int i = 0; i < count; i++) {
				...
				if ((headerDividers || !isHeader) && (footerDividers || !isFooter)) {
					...
					if (drawDividers && (bottom < listBottom)) {
						...
						if (areAllItemsSelectable || ((adapter.isEnabled(itemIndex) || (headerDividers && isHeader)
	...
	public static interface OnScrollListener {
		...
		public void onScroll(ZrcAbsListView view, int firstVisibleItem, int visibleItemCount, int
                totalItemCount);
	}
