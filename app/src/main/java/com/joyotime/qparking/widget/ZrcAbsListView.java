package com.joyotime.qparking.widget;
...
import com.joyotime.qparking.widget.ZrcListView.OnScrollListener;
import com.joyotime.qparking.widget.ZrcListView.OnScrollStateListener;
...
public abstract class ZrcAbsListView extends ZrcAdapterView<ListAdapter>
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		if (zrcHeader != null) {
			...
			if (needShowHeader) {
				boolean more = zrcHeader.draw(canvas, listPadding.left, viewTop + mFirstTopOffset,
		...
		if (childCount > 0 && (isRefreshing || isLoadingMore) && footer != null) {
			...
			if (firstPosition + childCount == mItemCount) {
				...
				if (lastBottom < viewBottom) {
					...
					if (more) {
						postInvalidate(listPadding.left, listPadding.top, getWidth() - listPadding.right,
		...
		post(new Runnable() {
			...
			public void run() {
				...
				if (mFirstPosition == 0) {
					...
					mFlingRunnable.startScroll(mFirstTop - newTop, (int) (Math.abs(newTop - mFirstTop) / mDensity) + 50,
							true);
	...
	private class FlingRunnable implements Runnable {
		...
		boolean scrollToAdjustViewsUpOrDown() {
			...
			final boolean isTooShort = childCount == mItemCount && lastBottom - firstTop < getHeight();
			final int bottomOffset = isTooShort ? firstTop - listPadding.top - mFirstTopOffset
					: lastBottom - getHeight() + listPadding.bottom + mLastBottomOffset;
