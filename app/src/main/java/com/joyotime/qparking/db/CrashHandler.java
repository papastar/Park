...
import com.joyotime.qparking.FeedBack;
...
public class CrashHandler implements UncaughtExceptionHandler {
	...
	public void init(Context context) {
		mdbDate = new MethodDate(context);
		mContext = context;
	...
	private void saveCrashInfo2File(Throwable ex) {
		...
		mdbDate.writeLogTxt("崩溃日志", sb.toString());

		Intent intent = new Intent();
		intent.putExtra("buglog", sb.toString());
		intent.setClass(mContext, FeedBack.class);
		mContext.startActivity(intent);

		// Intent data=new Intent(Intent.ACTION_SENDTO);
		// data.setData(Uri.parse("mailto:hzx@renrenparking.com"));
		// data.putExtra(Intent.EXTRA_SUBJECT, "崩溃日志");
