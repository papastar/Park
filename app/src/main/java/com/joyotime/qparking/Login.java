...
public class Login extends Activity {
	...
	private static Button login_btn_getyanzhengma;
		...
		public void handleMessage(Message msg) {
			...
			if (msg.what == 1) {
				...
				if (LoadUserData) {
					...
					startActivity(new Intent(Login.this, FlashApp.class));
...
//		});
		...
		login_btn_getyanzhengma.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				login_btn_getyanzhengma.setTextColor(ContextCompat.getColor(Login.this,R.color.forbidden_c));
		...
		login_et_phonenumber.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence s, int start, int before, int count) {
				...
				if (login_et_yanzhengma.getText().toString().length() == 6 && login_et_phonenumber.getText().toString().length() == 11) {
					login_btn_signin.setBackground(getResources().getDrawable(R.drawable.btn_signin));
					login_btn_signin.setClickable(true);
					InputMethodManager m = (InputMethodManager) getSystemService(Context.INPUT_METHOD_SERVICE);
					m.toggleSoftInput(0, InputMethodManager.HIDE_NOT_ALWAYS);
				} else {
					login_btn_signin.setBackground(getResources().getDrawable(R.drawable.btn_forbidden));
	...
	private void JosnAppv(String result) {
		...
		for (int i = 0; i < object.length(); i++) {
			...
			if (i == object.length() - 1) {
				...
				AppSetting.nowServiceSoftVersion = Integer.parseInt(jsObject.optString("softCode").toString());
				int hardCode = Integer.parseInt(jsObject.optString("hardCode").toString());
				AppSetting.Url_hardware = AppSetting.URL_STRING_Down + jsObject.optString("hardLocation");

				if (AppSetting.nowServiceSoftVersion > mdb.GetOneVersion(hardCode)) {
					DownLoadFiles("" + hardCode, "" + AppSetting.nowServiceSoftVersion, AppSetting.Url_hardware);
				}
			}
		...
		if (isauthcode) {
			...
			if ((_token_newString == null || _token_newString.equals("")) && !iswhileLoad) {
				...
				if (!DB.ValidationUser(_openId)) {
					DB.InstarUserDataLocal(_openId, name, token, avatar, cellphone, gender.equals("1") ? "男" : "女", province, city);
				} else {
				...
				for (int i = 0; i < object.length(); i++) {
					...
					String lockLng = jsObject.getJSONObject("location").getJSONArray("coordinates").optString(0);
					String lockLat = jsObject.getJSONObject("location").getJSONArray("coordinates").optString(1);
					String parkingAddress = UnicodeToString(jsObject.getJSONObject("parkingLot").optString("address"));
					String cityCode = jsObject.getJSONObject("parkingLot").optString("cityCode");
					String cityName = UnicodeToString(jsObject.getJSONObject("parkingLot").optString("cityName"));
					String parkingName = UnicodeToString(jsObject.getJSONObject("parkingLot").optString("name"));
					String lockAddress = UnicodeToString(jsObject.optString("address"));

					if (!DB.ValidationBlue(bluetooth, "1")) {
						if (DB.InstarBlueDBLocal(_id, bluetoothName, bluetooth, "", key, sn, locktoken, _openId, "1", i == 0 ? "1" : "0", lockLng, lockLat, lockAddress, cityName,
								parkingName, parkingAddress, cityCode, Botocellphone)) {
							DB.InstarVersion(hardware, "0", LockMain._OpenID, sn);
							// ShowToast("添加成功");
						}
					} else {
						DB.UpdateBlueDBLocal(bluetoothName, "", key, sn, locktoken, lockLng, lockLat, lockAddress, cityName, parkingName, parkingAddress, cityCode, Botocellphone);
					}
				}
				IsloadShare = true;
				new RequestTask().execute();
				...
				for (int i = 0; i < object.length(); i++) {
					...
					String bluetoothName = jsObject.getJSONObject("lock").optString("bluetoothName");
					...
					String lockLng = jsObject.getJSONObject("lock").getJSONObject("location").getJSONArray("coordinates").optString(0);
					String lockLat = jsObject.getJSONObject("lock").getJSONObject("location").getJSONArray("coordinates").optString(1);
					String parkingAddress = UnicodeToString(jsObject.getJSONObject("parkingLot").optString("address"));
					String cityCode = jsObject.getJSONObject("parkingLot").optString("cityCode");
					String cityName = UnicodeToString(jsObject.getJSONObject("parkingLot").optString("cityName"));
					String parkingName = UnicodeToString(jsObject.getJSONObject("parkingLot").optString("name"));
					...
					if (!DB.ValidationBlue(bluetooth, "0")) {
						if (DB.InstarBlueDBLocal(_id, bluetoothName, bluetooth, "", key, sn, locktoken, _openId, "0", count == 0 ? (i == 0 ? "1" : "0") : "0", lockLng, lockLat,
								lockAddress, cityName, parkingName, parkingAddress, cityCode, Botocellphone)) {
							// ShowToast("添加成功");
						}
					} else {
						DB.UpdateBlueDBLocal(bluetoothName, "", key, sn, locktoken, lockLng, lockLat, lockAddress, cityName, parkingName, parkingAddress, cityCode, Botocellphone);
	...
	public static class Login_WorkTheard extends Thread {
		...
		public void run() {
			...
			for (userinfo.moveToFirst(); !userinfo.isAfterLast(); userinfo.moveToNext()) {
				...
				LockMain._AddressString = userinfo.getString(provinceColumn) + " " + userinfo.getString(cityColumn);
	...
	public void DownLoadFiles(final String Finder, final String SoftWare, final String UrlHardware) {
		if (ConnectionChangeReceiver.IsNetWork) {
	...
	public void DownLoadFile(String Finder, String SoftWare, String FileName, String UrlHardware) {
		StringBuilder sb = new StringBuilder(Environment.getExternalStorageDirectory() + "/").append("QParking/" + Finder + "/");
		...
		DownloadManager downloadManager = (DownloadManager) contextmian.getSystemService(DOWNLOAD_SERVICE);
